...
func (a *AccessListTracer) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, scope *callCtx, rData []byte, depth int, err error) {
	stack := scope.stack
	if (op == SLOAD || op == SSTORE) && stack.len() >= 1 {
		slot := common.Hash{}
		slot.SetBytes(stack.data[stack.len()-1].Bytes())
		a.list.addSlot(scope.contract.Address(), slot)
	}
	if (op == EXTCODECOPY || op == EXTCODEHASH || op == EXTCODESIZE || op == BALANCE || op == SELFDESTRUCT) && stack.len() >= 1 {
		addr := common.Address{}
		addr.SetBytes(stack.data[stack.len()-1].Bytes())
	...
	if (op == DELEGATECALL || op == CALL || op == STATICCALL || op == CALLCODE) && stack.len() >= 5 {
		addr := common.Address{}
		addr.SetBytes(stack.data[stack.len()-2].Bytes())
		if _, ok := a.excl[addr]; !ok {
			a.list.addAddress(addr)
		}
	}
...
func (*AccessListTracer) CaptureFault(env *EVM, pc uint64, op OpCode, gas, cost uint64, scope *callCtx, depth int, err error) {
}
