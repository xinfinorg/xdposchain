...
func TestConfigApi(t *testing.T) {
	...
	assert.Equal(t, info.V2Config.MaxMasternodes, 18)
	assert.Equal(t, info.V2Config.CertThreshold, 0.667)
	assert.Equal(t, info.V2Config.MinePeriod, 2)
	assert.Equal(t, info.V2Config.TimeoutSyncThreshold, 2)
}
