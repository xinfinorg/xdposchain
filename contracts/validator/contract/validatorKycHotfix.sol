...
// This contract is a hotfix for current binding issue.
...
contract XDCValidator {
    ...
    // once :  hotfix for unchanged constructor
    function onceHotfix() external {
        require(msg.sender == validatorsState[candidates[0]].owner);
        owners.push(msg.sender);
        for (uint i=0;i<candidates.length;i++){
            ownerToCandidate[msg.sender].push(candidates[i]);    
        }
